...
@Component({
    template: `<input type="tel" priceFormatter>`
})
class TestComponent {
}

describe('PriceFormatterDirective', () => {
   let fixture: ComponentFixture<TestComponent>;
    let debugElement: DebugElement;
    let component: TestComponent;

    let priceFormatterDirective: PriceFormatterDirective;
    let inputBox: HTMLInputElement;
    let inputEl: DebugElement;
    let control: NgControl;

    beforeEach(() => {
        priceFormatterDirective = new PriceFormatterDirective(new ElementRef(null), control, new PricePipe());

        TestBed.configureTestingModule({
            declarations: [PriceFormatterDirective, TestComponent],
            imports: [ReactiveFormsModule, FormsModule],
            providers: [PricePipe, NgControl]
        });
